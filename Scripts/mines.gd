...
#List of the odds of the minerals
var MineralList = {
	Gold = 0.75,
	Iron = 3,
	Emerald = 0.5,
...
func _ready():
	...
	#Only ducks over 7 years old and not in hospital can mine
...
func _on_duck_list_item_clicked(index: int, _at_position: Vector2, _mouse_button_index: int) -> void:
	#Sets variables to be whats located in the saveData
...
func _on_continue_pressed() -> void:
	#Sets the visuals and sets the current tab to waiting tab
	if saveDataValues.miningValues.get("selectedDuck") != "":
		...
		#Error just in case there is no duck selected
...
func obtainMinerals():
	#Loops through the mineral list and solves for how much of the mineral the duck gets based a randomly generated float and the mineral's odds
	current_tab = 2
	obtainedMinerals = {}
...
func duckGotInjured():
	#sets the hospital values if the duck gets injured
...
func miniGameOptionPicked(pathChosen: int):
	#Based on what path was chosen, will get that value and sets up what happens if you pick the positive option, negative option, and middle option
	var v = randf()
	if pathChosen == 0:
